...
- Complete local inference and review on macOS first while preserving a portable engine contract for a later iOS/Android change.
...
- **Native inference threatens host-only engine builds** → Isolate target-specific packaging and check `cargo build --workspace` without the mobile toolchain.
...
- Which consented fixed-camera singles and doubles recordings, devices, and acceptance thresholds will be used for the runtime/model comparison?
- Which exact LiteRT packaging path works on macOS while preserving a host-buildable Rust engine and the single API facade? Mobile packaging is deferred.
