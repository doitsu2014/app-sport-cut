...
Court calibration is complete, but the engine cannot yet locate players or produce the court-aware tracks needed by rally segmentation and later highlight ranking. This change fills that Wave 2 gap on macOS first while keeping uncertain detections and player counts visible as uncertain evidence rather than presenting them as facts.
...
- Deliver and evaluate the first working path on macOS. iOS and Android runtime packaging and device verification follow in a later change; the shared engine and track artifact remain portable.
...
- **Client and bridge:** generated bindings are regenerated, while hand-written Dart wrappers and a focused macOS analysis/review surface present count, tracks, progress, and uncertainty. Mobile packaging is deferred.
