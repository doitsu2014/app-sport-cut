...
- [x] 1.1 Obtain and document the available consented fixed-camera doubles clip (off-angle, distant players, spectators, slight occlusion); singles footage, second court orientation, and rotation-tagged recordings are deferred (see design D1)
...
- [x] 1.4 Select and pin the exact macOS runtime packaging path and model weight bytes, and record separate runtime, weights, and training-dataset license and redistribution verdicts in `docs/legal/dependency-register.md`; keep unresolved components off the shipping path
- [x] 1.5 Record the selected model's source, version, checksum, input/output format, and intended use under `models/`, and confirm inference can run without network access
...
- [x] 2.2 Implement the approved `PersonDetector` backend behind the existing vision trait, with target-specific packaging isolated so `core/` remains host-buildable and hand-written Rust remains free of `unsafe`
