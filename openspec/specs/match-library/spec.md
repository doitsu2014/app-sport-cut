...
## Purpose
The application-owned local catalog and the match library experience: taking
durable custody of a recording at import and recording the app-owned copy,
populating media metadata from the engine, browsing stored matches, playing them
back offline, holding the rally, score, clip, and export records a review session
writes, and deleting a match without ever touching the file the user selected.
## Requirements
