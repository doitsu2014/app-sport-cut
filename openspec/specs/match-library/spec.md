...
## Purpose
The application-owned local catalog and the match library experience: importing a
recording by reference rather than copying it, populating media metadata from the
engine, browsing stored matches, playing them back offline, and deleting them
